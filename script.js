/* ===================== Cosmic backdrop (Premium Neon Black Hole) ===================== */
(function backdrop() {
  ...
  // Exact Hex Palette from your visual reference guide
  const COLORS = {
    WHITE: "255, 255, 255",
  ...
  function updateHolePosition() {
    ...
    // Safety check: Ensure the black hole isn't invisible/zero-size
  ...
  // Lensing Math: Projects 3D-like wrap-around coordinates into 2D space
  function project(a, dist, isFar, t) {
    ...
    if (isFar) {
      // Create the "bowed" wrap-around lensing seen in the reference
      const pull = Math.pow(Math.abs(sinA), 1.4) * (hole.r * 0.95);
      y -= (sinA < 0 ? 1 : -1) * pull;
    }

    // Plasma edge turbulence
    const noise = Math.sin(a * 4 + t * 0.0015) * (hole.r * 0.05);
    x += noise; y += noise;
  ...
  function drawDisk(t, isFar) {
    ...
    for (let l = 0; l < layers; l++) {
      ...
      // Intensive neon bloom using shadow system
      ctx.shadowBlur = 18;
  ...
  function drawHole(t) {
    ...
    // 1. Extreme Ambient Neon Bloom
    const bloom = ctx.createRadialGradient(cx, cy, r, cx, cy, r * 6);
    bloom.addColorStop(0, `rgba(${COLORS.MAGENTA}, 0.25)`);
    bloom.addColorStop(0.5, `rgba(${COLORS.PURPLE}, 0.1)`);
    bloom.addColorStop(1, "rgba(0,0,0,0)");
    ctx.fillStyle = bloom;
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    // 2. Far part of the disk (wrapping behind)
    drawDisk(time, true);

    // 3. Central Event Horizon (Solid black void with photon ring)
    ctx.globalCompositeOperation = "source-over";
    const voidGrad = ctx.createRadialGradient(cx, cy, r * 0.88, cx, cy, r);
    voidGrad.addColorStop(0, `rgb(${VOID_COLOR})`);
    voidGrad.addColorStop(0.92, `rgb(${VOID_COLOR})`);
    voidGrad.addColorStop(0.96, `rgba(${COLORS.WHITE}, 0.9)`); // Internal neon rim
    voidGrad.addColorStop(1, "rgba(255,255,255,0)");
    ctx.fillStyle = voidGrad;
    ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI * 2); ctx.fill();

    // 4. Near part of the disk (In front of the sphere)
    ctx.globalCompositeOperation = "lighter";
    drawDisk(time, false);
    ...
    const jet = ctx.createLinearGradient(0, -r, 0, -r * 4.5);
    jet.addColorStop(0, `rgba(${COLORS.WHITE}, 0.35)`);
    jet.addColorStop(1, "rgba(0,0,0,0)");
    ctx.fillStyle = jet;
    for(let d of [-1, 1]) {
      ctx.beginPath(); ctx.moveTo(-r*0.08, 0); ctx.lineTo(r*0.08, 0); ctx.lineTo(0, d*r*4.5); ctx.fill();
    }
    ctx.restore();
    ctx.globalCompositeOperation = "source-over";
  }
...
/* ===================== FULL SITE LOGIC RESTORED (Search, 3D, Specs) ===================== */
...
if (els.search) {
  ...
  els.search.addEventListener("input", () => {
    ...
    els.suggestions.textContent = hit ? `showing ${hit.data.name}…` : "no match yet — try an element symbol/name or a small molecule formula";
...
function showSubject(hit) {
  ...
  if (hit.type === "element") {
    ...
    els.viewerNote.textContent = `3D atom model — magenta protons and orange neutrons packed in the nucleus, blue electrons circling on glowing shells. Drag to rotate, pinch/scroll to zoom, tap a particle to inspect it. Not to true relative scale.`;
...
if (els.chatForm) {
  els.chatForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      pending.textContent = data.reply || "Something went wrong — try again.";
