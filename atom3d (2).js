...
function drawAtom3D(symbol, elData) {
  ...
  let downPos = null, moved = false, selected = null;
  ...
  // Use pointerdown/pointermove to track drag distance, then raycast on
  // 'click' which fires AFTER OrbitControls has fully resolved its own
  // pointerup — prevents OrbitControls getting stuck in drag state.
  ...
  renderer.domElement.addEventListener('click', e => {
    if (moved) { moved = false; downPos = null; return; }
