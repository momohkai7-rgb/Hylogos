...
function atom3dBuildElectrons(ctx, Z){
  ...
  shellNs.forEach(n => {
    ...
    info.forEach((d, idx) => {
      ...
      const mesh = new THREE.Mesh(new THREE.SphereGeometry(ATOM3D_ELECTRON_R, 16, 16), mat);
...
function drawAtom3D(symbol, elData) {
  ...
  function clearSelection(){
    if (selected){
      ...
      const go = selected.userData && selected.userData.glowOuter;
      if (g && g.scale && g.material){
      ...
      if (selected.material) selected.material.emissiveIntensity = selected.userData.baseEmissive;
      selected.scale.set(1,1,1);
      // Resume the spinGroup if we paused it
  ...
  function selectMesh(m){
    ...
    // Pause this electron's spinGroup so the mesh holds still while selected
    if (isElectron && m.parent && m.parent.userData.speed !== undefined){
...
window.drawCompound3D = function(data) {
    ...
    data.atoms.forEach(a => {
        ...
        mesh.userData = { title: ELEMENTS[a.el].name, desc: "An atom in this molecule." };
        scene.add(mesh);
    ...
    animate();
    window.threeScene = { renderer }; // Save so it can be cleared later
};
    ...
    for (let x = -1; x <= 1; x++) {
        for (let y = -1; y <= 1; y++) {
            for (let z = -1; z <= 1; z++) {
                ...
                const el = isMinor ? data.composition[1].el : data.composition[0].el;
                const atom = new THREE.Mesh(
                    new THREE.SphereGeometry(0.5, 24, 24),
                    new THREE.MeshStandardMaterial({ color: ATOM_COLOR[el] || 0x888888, metalness: 1, roughness: 0.1 })
                );
