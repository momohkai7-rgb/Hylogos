...
function drawAtom3D(symbol, elData) {
  ...
  function selectMesh(m){
    ...
    selected = m;
    controls.enabled = false; // stop OrbitControls capturing further pointer events
    const isElectron = m.userData.kind === 'electron';
    const s = isElectron ? 1.4 : 1.25;
    m.scale.set(s,s,s);
    m.material.emissiveIntensity = m.userData.baseEmissive * 1.3;
  ...
  function animate(t) {
    ...
    if (selected && selected.userData.glow){
