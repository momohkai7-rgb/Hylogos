...
(function backdrop() {
  ...
  const VOID_RGB = "2, 0, 8"; // Deep space black-purple
  ...
  function updateHolePosition() {
    ...
    // Bulletproof Fallback: If search bar position is not yet known
  ...
  // Lensing Projection: Warps coordinates to wrap around the center sphere
  function project(a, dist, isFar, t) {
    ...
    if (isFar) {
      // Create the iconic vertical "warp" seen in the reference
      const pull = Math.pow(Math.abs(sinA), 1.5) * (hole.r * 1.0);
      y -= (sinA < 0 ? 1 : -1) * pull;
    }

    // Plasma edge turbulence
    const noise = Math.sin(a * 4 + t * 0.0018) * (hole.r * 0.05);
    x += noise; y += noise;
  ...
  function drawDisk(t, isFar) {
    ...
    for (let l = 0; l < layers; l++) {
      const rIn = hole.r * (0.94 + l * 0.24);
      const rOut = rIn + (hole.r * 0.5);
      const alpha = (0.2 - l * 0.03) * (isFar ? 0.7 : 1);
      ...
      for (let i = 0; i <= steps; i++) {
        ...
        if ((Math.sin(a) < 0) !== isFar) { hasStarted = false; continue; }
        const p = project(a, rOut, isFar, t);
        if (!hasStarted) { ctx.moveTo(p.x, p.y); hasStarted = true; } 
        else { ctx.lineTo(p.x, p.y); }
      }
      
      // Intensive Crisp Bloom
      ctx.shadowBlur = 15;
      ctx.shadowColor = `rgba(${col}, ${alpha})`;
      ctx.strokeStyle = `rgba(${col}, ${alpha})`;
      ctx.lineWidth = hole.r * 0.58;
      ctx.lineCap = "round";
      ctx.stroke();
    }
    ctx.restore();
  ...
  function drawHole(t) {
    ...
    // 1. Extreme Neon Bloom atmosphere
