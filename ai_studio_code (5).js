...
(function backdrop() {
  ...
  // Exact Palette from your "Color Palette" guide
  const COLORS = {
    WHITE: "255, 255, 255",
  ...
  function project(angle, dist, isFar, t) {
    const sinA = Math.sin(angle);
    let x = Math.cos(angle) * dist;
    let y = sinA * dist * DISK_SQUASH;

    if (isFar) {
      // Create the vertical "S-curve" wrap seen in the reference
      const pull = Math.pow(Math.abs(sinA), 1.5) * (hole.r * 0.95);
      y -= (sinA < 0 ? 1 : -1) * pull;
    }

    // Plasma flow turbulence
    const noise = Math.sin(angle * 5 + t * 0.002) * (hole.r * 0.04);
    x += noise; y += noise;
  ...
  function drawDiskLayer(t, isFar) {
    const segments = 180;
    const layers = 6;
    ...
    for (let l = 0; l < layers; l++) {
      ...
      const alpha = (0.22 - l * 0.035) * (isFar ? 0.7 : 1);
      ...
      for (let i = 0; i <= segments; i++) {
        ...
        if (ctx.getPathStackSize === 0) ctx.moveTo(p.x, p.y);
  ...
  function drawHole(t) {
    ...
    // 1. Vibrant Background Neon Bloom
    ...
    // 2. Far side of Disk (Background wrap)
    drawDiskLayer(time, true);

    // 3. Central Event Horizon (Perfectly Circular Void)
    ctx.globalCompositeOperation = "source-over";
    const voidGrad = ctx.createRadialGradient(cx, cy, r * 0.88, cx, cy, r);
    voidGrad.addColorStop(0, `rgb(${VOID})`);
    voidGrad.addColorStop(0.92, `rgb(${VOID})`);
    voidGrad.addColorStop(0.96, `rgba(${COLORS.WHITE}, 0.9)`); // Internal photon ring
    ...
    // 4. Near side of Disk (Foreground wrap)
    ctx.globalCompositeOperation = "lighter";
    drawDiskLayer(time, false);
    
    // 5. Polar Energy Pulse (Energy streaming perpendicular to disk)
    ctx.save();
    ctx.translate(cx, cy); ctx.rotate(DISK_TILT);
    const jet = ctx.createLinearGradient(0, -r, 0, -r * 4);
    jet.addColorStop(0, `rgba(${COLORS.WHITE}, 0.3)`);
    jet.addColorStop(1, "rgba(0,0,0,0)");
    ctx.fillStyle = jet;
    for(let d of [-1, 1]) {
      ctx.beginPath(); ctx.moveTo(-r*0.1, 0); ctx.lineTo(r*0.1, 0); ctx.lineTo(0, d*r*4); ctx.fill();
    }
    ctx.restore();
    ctx.globalCompositeOperation = "source-over";
  }
...
/* ===================== State & Site Logic (Restored to Original Size) ===================== */
...
function drawBohr(el) {
  ...
  function size() { canvas.width = host.clientWidth; canvas.height = host.clientHeight; }
  size();
  ...
  function frame(t) {
    ...
    grad.addColorStop(0, "#fff2d6"); grad.addColorStop(1, "#ffb454");
    ctx.fillStyle = grad;
    ctx.beginPath(); ctx.arc(cx(), cy(), nucR, 0, Math.PI * 2); ctx.fill();

    el.shells.forEach((count, i) => {
      const r = gap() * (i + 1.6);
      ctx.strokeStyle = "rgba(232,228,240,0.18)"; ctx.lineWidth = 1;
      ...
      for (let e = 0; e < count; e++) {
        ...
        ctx.fillStyle = "#7fd9ff"; ctx.beginPath(); ctx.arc(ex, ey, 3.2, 0, Math.PI * 2); ctx.fill();
      }
    });
