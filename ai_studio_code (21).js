...
function showSubject(hit) {
  ...
  if (hit.type === "element") {
    ...
    const elCount = Object.keys(hit.data.elements).length;
    els.subjectName.textContent = `${hit.data.name} (${hit.data.formula})`;
    els.subjectMeta.textContent = `${elCount} elements · alloy`;
    els.bohrCanvas.style.display = "none";
    els.threeHost.style.display = "block";
    els.viewerNote.textContent =
      `Packed metallic lattice — drag to rotate, pinch/scroll to zoom, tap a sphere to see which element it is and its share of the alloy. Illustrative, not a true crystal structure.`;
    ...
    els.threeHost.style.display = "block";
    els.viewerNote.textContent =
      `Ball-and-stick model — drag to rotate, pinch/scroll to zoom, tap an atom or bond to inspect it. Bond lengths and angles are idealized for shape, not to exact scale.`;
