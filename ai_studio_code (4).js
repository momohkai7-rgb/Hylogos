...
(function backdrop() {
  ...
  // Gravitational lensing projection: warps paths around the event horizon
  function project(a, dist, isFar, t) {
    ...
    if (isFar) {
      // Warp the back half of the disk to create the vertical arcs
      const pull = Math.pow(Math.abs(sinA), 1.4) * (hole.r * 0.9);
      y -= (sinA < 0 ? 1 : -1) * pull;
    }

    // Plasma turbulence
    const noise = Math.sin(a * 5 + t * 0.002) * (hole.r * 0.05);
    x += noise; y += noise;
  ...
  function drawDisk(t, isFar) {
    ...
    for (let l = 0; l < layers; l++) {
      ...
      const alpha = (0.16 - l * 0.025) * (isFar ? 0.6 : 1);
  ...
  function drawHole(t) {
    ...
    voidGrad.addColorStop(0.96, `rgba(${COLORS.WHITE}, 0.8)`); // Photon ring
    voidGrad.addColorStop(1, "rgba(255,255,255,0)");
    ctx.fillStyle = voidGrad;
    ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI * 2); ctx.fill();

    // 4. Near lensing arcs (Foreground)
    ctx.globalCompositeOperation = "lighter";
    drawDisk(time, false);
    
    // 5. Plasma energy pulse at poles
    ctx.save();
